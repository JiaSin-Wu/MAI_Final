...
from src.module.dict_moe_direct import DictMoEDirect
    ...
    def load_model(self):
        ...
        with timeit_context("Building DictMoEDirect model"):
            ...
            for layer_idx in range(model.model.visual.transformer.layers):
                ...
                log.info(f"Replacing MLP of layer {layer_idx} with DictMoEDirect.")
                # Use DictMoEDirect: direct weighted ensemble of distilled expert weights
                # No base model, no task vectors - just weighted combination of expert MLPs
                model.model.visual.transformer.resblocks[layer_idx].mlp = DictMoEDirect(
